<template>
  <view class="container">
    ...
    <view v-else-if="room" class="result-container">
      ...
      <!-- 公共牌展示 -->
      <view v-if="room.communityCards && room.communityCards.length > 0" class="community-cards">
        ...
        >
          ...
          <view class="player-cards">
            ...
            <view class="cards-row">
              <Card
                v-for="(card, idx) in (player.hand || player.cards || [])"
                :key="idx"
                :suit="getCardSuit(card)"
                :rank="getCardRank(card)"
                :face-up="true"
      ...
      <!-- 底池分配 -->
      <view v-if="room.pot && room.pot > 0" class="pot-distribution">
...
export default {
  ...
  methods: {
    ...
    getHandType(player) {
      ...
      if (!player.hand && (!player.cards || player.cards.length === 0)) return '未发牌'
      // TODO: 调用 hand-evaluator.js 评估牌型
      return '待计算'
    },
